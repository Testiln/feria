...
export default function HeroSection({ onGoToProducts }) {
  return (
    <section className="relative w-full min-h-[100svh] bg-[#e3d7d7] flex items-center justify-center overflow-hidden px-6 lg:px-20 py-20">
      <div className="absolute top-0 right-0 w-[50vw] h-[100vh] bg-[#d4c4c4] rounded-l-full opacity-20 transform translate-x-1/4"></div>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left fade-in w-full">
          ...
          <button onClick={onGoToProducts} className="bg-white text-[#C62828] hover:bg-transparent hover:text-white hover:border-white border-2 border-transparent transition-all duration-300 font-poppins font-bold uppercase tracking-widest px-10 py-5 rounded-full text-lg shadow-[0_10px_20px_rgba(0,0,0,0.2)] flex items-center gap-3 group mx-auto lg:mx-0">
        ...
        <div className="relative w-full h-[50vh] lg:h-[80vh] flex justify-center items-center fade-in" style={{animationDelay: '0.2s'}}>
          <div className="absolute inset-0 bg-[#d4c4c4] rounded-[2rem] transform rotate-3 scale-95 opacity-50"></div>
          <img src="/landing1.png" alt="Muebles nuevos Jamar" className="w-full h-full object-cover rounded-[2rem] shadow-2xl relative z-10" />
        </div>
      </div>
    </section>
  );
